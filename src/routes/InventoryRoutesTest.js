...
async function changeLikes(recipeId, add) {
  ...
  try {
    ...
    const Recipe = await client.db('<dbname>').collection('tempRecipes');
    let result;
    if (add) {
 ...
 * view inventory
 */
router.get('/inventory2', requireAuth, async (req, res) => {
  try {
 ...
 */
router.post('/inventory2', requireAuth, async (req, res) => {
  try {
 ...
 * view saved recipes
 */
router.get('/savedRecipes2', requireAuth, async (req, res) => {
  try {
    let i;
    const result = [];
 ...
 */
router.post('/savedRecipes2', requireAuth, async (req, res) => {
  try {
 ...
 */
router.post('/recipeClicked2', requireAuth, async (req, res) => {
  try {
 ...
 */
router.get('/allIngredients2', requireAuth, async (req, res) => {
  const client = new MongoClient(mongoUri);
  try {
 ...
 */
router.get('/allRecipes2', requireAuth, async (req, res) => {
  const client = new MongoClient(mongoUri);
  try {
    await client.connect();
    const cursor = await client.db('<dbname>').collection('tempRecipes').find();
    const result = await cursor.toArray();
    return res.send(result);
  } catch (e) {
    console.error(e);
